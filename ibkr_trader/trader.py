    ...
    def execute_trade(self, trade_decision: Dict) -> Dict:
        ...
        if action == 'buy': # Handle single leg option buy
            ...
            try:
                ...
                # Additional checks before execution
                if quantity <= 0:
                    self.logger.error(f"Invalid quantity for vertical spread trade: {quantity}")
                    return {'executed': False, 'reason': 'invalid_quantity'}
                
                if net_credit_estimate <= 0:
                    self.logger.error(f"Invalid credit estimate for vertical spread trade: {net_credit_estimate}")
                    return {'executed': False, 'reason': 'invalid_credit_estimate'}
                
                # Convert option_type to IB format
                right = 'C' if option_type.lower() == 'call' else 'P'
                
                # For vertical spreads, we need to determine if we're selling or buying the spread
                # Credit spreads (bull put, bear call): We SELL the spread to receive a credit
                action = "SELL"  # Default for credit spreads
                
                # Calculate the minimum acceptable credit (95% of estimate)
                min_acceptable_credit = net_credit_estimate * 0.95
                
                # Start with a limit price slightly above our estimate for better fills
                initial_limit_price = net_credit_estimate * 1.02  # Start with 102% of estimate
                
                self.logger.info(f"Attempting {spread_type} spread execution with initial limit price: ${initial_limit_price:.2f} (estimate: ${net_credit_estimate:.2f})")
                
                # Create the vertical spread order
                # For bull put: sell higher strike put, buy lower strike put
                # For bear call: sell lower strike call, buy higher strike call
                trade = self.ib_connector.submit_vertical_spread_order(
                    symbol=symbol,
                    expiry=expiry,
                ...
                while not filled and current_attempt <= max_attempts:
                    ...
                    if trade and trade.orderStatus and trade.orderStatus.status == 'Filled':
                        filled = True
                        self.logger.info(f"Vertical spread filled at attempt {current_attempt} with price ${trade.orderStatus.avgFillPrice:.2f}")
                        break
                ...
                # If we couldn't fill the order after all attempts
                if not filled:
                    self.logger.warning("Failed to fill vertical spread order after multiple attempts")
                ...
                # If we got here, the order was filled
                if not trade or not trade.orderStatus:
                    self.logger.error(f"Order status unavailable for filled vertical spread")
                    return {'executed': False, 'reason': 'order_status_unavailable'}
                
                self.day_trade_count += 1  # Counts as one trade
                self.last_trade_time = datetime.now()
                ...
                # Verify we got a reasonable price
                if avg_fill_price < min_acceptable_credit:
                    self.logger.warning(f"Vertical spread filled at ${avg_fill_price:.2f}, below minimum acceptable ${min_acceptable_credit:.2f}")
                
                # Construct a symbol for the spread
                spread_symbol = f"{symbol}_{spread_type}_{expiry}_{long_strike:.0f}{right}_{short_strike:.0f}{right}"
                ...
                self.risk_manager.add_position(
                    ...
                    option_data={  # Store all relevant details for the spread
                        'underlying': symbol,
                ...
                self.logger.info(f"Executed {spread_type} spread: {quantity} {spread_symbol} for a credit of ${avg_fill_price:.2f} per spread")
                self.logger.info(f"Fill quality: {(avg_fill_price / net_credit_estimate) * 100:.1f}% of estimated credit")
                ...
                self.logger.error(f"Error executing vertical spread trade: {e}", exc_info=True)
                return {'executed': False, 'reason': str(e)}
